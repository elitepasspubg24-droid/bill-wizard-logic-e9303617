...
function RatesPage() {
  ...
  const addFactory = useMutation({
    mutationFn: async () => {
      ...
      // Casted to any to prevent strict type failures during build
  ...
  const saveAll = useMutation({
    mutationFn: async () => {
      ...
      for (const f of factories) {
        ...
        // Casted to any to allow updating properties not yet reflected in generated types
        const { error } = await (supabase.from("factories") as any).update(patch).eq("id", f.id);
        if (error) failures.push(`${f.name}: ${error.message}`);
      }
      if (failures.length) throw new Error(failures.join(" | "));
    },
